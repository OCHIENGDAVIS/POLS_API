...
class TestParty(BaseTest):
    ...
    def test_create_party(self):
        ...
        with self.app_context ():
            response = self.app.post("/api/v1/parties", data=json.dumps(self.party_test_data), content_type="application/json")
            self.assertEqual(response.status_code, 201, msg="POST method did not return a 201 created code")
            response_msg = json.loads(response.data.decode("UTF-8"))
            self.assertEqual(response_msg['status'], 201, msg="status not in response, expected status")
    ...
    def test_create_party_already_exists(self):
        ...
        with self.app_context():
            ...
            self.assertEqual(response.status_code, 400,  msg="POST creating an existing party did not return a 400 bad request.")
    ...
    def test_get_party_not_found(self):
        ...
        with self.app_context():
            ...
            self.assertEqual(response.status_code, 404, msg="Error Party found, expectedd a 404 not found")
    ...
    def test_delete_party(self):
        ...
        with self.app_context():
            ...
            response = self.app.post("/api/v1/parties", data = json.dumps(party_to_delete), content_type="application/json")
